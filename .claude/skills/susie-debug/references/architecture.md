...
Derived by reading the code on **`origin/latency-eval`** — the branch engine work
actually lands on — measured 2026-09-14. Line counts drift; ownership boundaries
drift far slower.

⚠️ Do not take these numbers from a clinic branch. `jv-v1-onboarding` carries a
`connection.py` 37% smaller and a `receptionist_tools.py` roughly half the size,
so measuring there understates the hazard surface by about half.
...
Caller
 └─ Twilio PSTN
     └─ app/routes/twilio.py            2,002   webhook, signature check, TwiML, greeting
         └─ app/media_streams/router.py   718   /ms/incoming TwiML + WS route
             └─ connection.py          19,003   WS lifecycle, turn taking, watchdogs
                 ├─ audio_in.py           128   Twilio µ-law 8k → PCM16 16k
                 ├─ stt_stream.py       1,305   AssemblyAI v3 streaming ASR
                 ...
                 │   ├─ llm_stream.py    9,169  Anthropic/OpenAI/Groq streaming
                 ...
                 ├─ tts_stream.py       1,097   ElevenLabs streaming TTS
...
| `handle_transcript()` | 5894 | **16,010** | every caller utterance |
| `_detect_intent()` | 21904 | — | intent classification |
| `_handle_mid_flow_interrupt()` | 22552 | 1,167 | FAQ/tangent mid-booking |
| `_handle_slot_confirmation()` | 23719 | — | yes/no/day-escape at a slot |
| `_handle_readback_confirmation()` | 24176 | — | final booking readback |
...
**Hazard:** `handle_transcript` is one 16,010-line async method. The policy is
...
**Hazard:** **127 of its 163** `except` clauses are `except Exception` or bare. The
