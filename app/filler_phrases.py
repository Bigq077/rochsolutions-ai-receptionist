...
# the appointment is already found and we're cancelling it) — JV P17 parity.
...
def confirm_write_filler(session: dict) -> Optional[str]:
    ...
    NOTE: this REPLACES the generic ack filler at that one moment; it never
    removes fillers.  Everywhere else the normal FILLER_PHRASES pool still
    plays exactly as before (the caller must never hear dead air).
    ...
    again — which can re-open the confirmation (the Marcus spiral).  Worse, that
    stray utterance lands as a new transcript and cancels the in-flight
    run_turn, which is where book_appointment is executing.  Playing a
