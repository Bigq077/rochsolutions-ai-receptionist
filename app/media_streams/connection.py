...
def _clamp_play_secs(play_secs: float, text: str) -> float:
    """Refuse a play duration that the text could not possibly take to say.

    ``play_secs`` is derived from ``_tts_bytes_sent``, one mutable counter
    shared by the TTS producer, the send loop and several teardown paths, and
    zeroed only when a done-sentinel is dequeued.  Bytes from audio that never
    places a sentinel, or that is drained by a teardown path which does not
    reset the counter, are charged to the next chunk that does.

    The failure that follows is not a slightly long timer.  The terminal
    ...
    **no watchdog is armed at all**.  Measured on CA268397d4 (22 Aug, Theorem):
    ...
    return max_plausible

# Pre-synthesised filler clips, anchored to the repo root rather than the
# process CWD (app/media_streams/connection.py → parents[2]). Generated by
# scripts/synthesise_filler.py and committed, because Render deploys from git.
_AUDIO_CLIPS_DIR: Path = Path(__file__).resolve().parents[2] / "audio_clips"
    ...
    async def _speech_recovery(self, recovery_step: int = -1, q_gen: int = 0) -> None:
        ...
        if _state in ("LOOKUP_RESCHEDULE", "LOOKUP_CANCEL"):
            ...
            if (_sr_sess or {}).get("v3_location_q_active"):
                ...
                # TWO RUNGS ONLY (owner instruction, 2026-08-04, restated).
                # The open question is the ask; the FIRST re-ask goes straight
                # to the keypad. The biased "did you say the Awlstuh clinic?"
                # confirm that used to sit between them is gone: it guessed a
                # clinic at a caller who had just failed to be understood, and
                # it spent the one turn that should have been deterministic.
                ...
                if _sr_sess:
                    ...
                    logger.info(
                        "[ms_conn v3] silence ladder → DTMF keypad"
                        " (first re-ask)",
                    )
                if _sr_sess:
        ...
        while True:
            ...
            if _state in ("GREETING", "DETECT_INTENT", ""):
                ...
                # next response to the existing yes/no confirmation handler.
                    ...
                    # TWO RUNGS ONLY (owner instruction, 2026-08-04, restated).
                    # Straight to the keypad on the first re-ask. The biased
                    # confirm is gone — after a caller has already not been
                    # understood once, guessing Alcester at them is the wrong
                    # move, and it consumed the turn that should have been
                    # deterministic. Clearing v3_location_q_active stops the
                    # ladder here, as before.
                    phrase = _LOC_RUNG3_DTMF
                    if _sess is not None:
                        ...
                        logger.info(
                            "[ms_conn v3] watchdog → DTMF keypad (first re-ask)",
                        )
                else:
...
class WebSocketCallHandler:
    ...
    def __init__(self, websocket: WebSocket) -> None:
        ...
        # the repo root loaded nothing and FillerGuard disabled itself with a
        # warning — the caller heard 3–6 s of silence on every availability
        # lookup and nothing in the call surfaced it. The clips must also be
                ...
                while not self._stop_event.is_set():
                    ...
                    try:
                        ...
                        if _v3_gate_fired:
                            ...
                            if self.session.get(
                                ...
                                if _confirmed_loc:
                                    ...
                                    if _resolved != "unknown":
                                        ...
                                        # TWO RUNGS ONLY (owner instruction,
                                        # 2026-08-04, restated). Straight to
                                        # the keypad. This is the site that
                                        # fires when STT produced something
                                        # Haiku could not resolve to a clinic —
                                        # exactly the case where guessing
                                        # Alcester at the caller is worst, and
                                        # where a keypad answer is certain.
                            ...
                            # And whether the caller has stated an age below
                            # the clinic's minimum. Same phase, same reason:
                            # the prompt's decline is not deterministic, so
                            # without this the turn drifts back to booking.
            ...
            while not self._stop_event.is_set():
                ...
                if b64_payload is _TTS_DONE_SENTINEL:
                    ...
                    if text and play_secs > 0.01:
                        # -- Impossible-duration clamp ---------------------
                        # play_secs comes from _tts_bytes_sent, a single
                        # mutable counter shared by the producer, this loop
                        # and several teardown paths, and zeroed only when a
                        # sentinel is dequeued.  Audio injected without a
                        # sentinel (_send_ulaw -- the filler clip and the
                        # breath gap) and audio drained on a teardown path
                        # that does not reset the counter both leak their
                        # bytes into the NEXT chunk that does place one.
                        #
                        # The consequence is not a slightly long timer.  The
                        # terminal chunk's callback is scheduled so far out
                        ...
                        play_secs = _clamp_play_secs(play_secs, text)
            ...
            while not self._stop_event.is_set():
                ...
                if _safety_net_count == 1:
                    ...
                    if self.session.get("v3_location_q_active"):
                        ...
                        # 2026-06-12 12:23).
                        #
                        # TWO RUNGS ONLY (owner instruction, 2026-08-04,
                        # restated). This used to mirror the watchdog's rung-1
                        # biased confirm; that rung no longer exists, and
                        # leaving it here would make this the one path that
                        # still guesses Alcester. The trigger for this branch is
                        # literally "STT can't catch the clinic name", which is
                        # the strongest case in the file for going to the keypad
                        # rather than asking again in words.
