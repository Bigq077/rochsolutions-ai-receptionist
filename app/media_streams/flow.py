...
class FlowEngine:
    ...
    async def handle_transcript(self, transcript: str) -> None:
        ...
        # Runs before current_step() so it fires even when flow_step has
        # drifted past the end of GENERAL_QUERY_FLOW.
        # For every other state this block is a strict no-op.
        # ════════════════════════════════════════════════════════════════════
        _rescue_text  = transcript.strip().lower()
        _rescue_state = self.session.get("state")
        print("[TOP ENTRY]", {"state": _rescue_state, "text": _rescue_text[:40]})
        if _rescue_state not in ("ANSWER_GENERAL", "GENERAL_BOOKING_OFFER"):
            pass  # absolutely no-op — fall through to original flow unchanged
        else:
            try:
                print("[GENERAL RESCUE ENGAGED]", {"state": _rescue_state, "text": _rescue_text[:40]})
                ...
                print("[GENERAL RESCUE ERROR]", repr(_rg_exc), {"state": _rescue_state, "text": _rescue_text[:40]})
