...
# A model turn that asks for a name PART -- Gate 5n-f counts these while
# only one word of the name is on record.
_NAME_PART_ASK_RE = re.compile(
    r"\b(?:first name|surname|last name|full name|both names|your name)\b[^.!?]*\?"
    r"|\bis \w+ your (?:first name|surname|last name)\b",
    re.IGNORECASE,
)
...
def _is_name_rejection(session: Dict[str, Any], caller: str) -> bool:
    ...
    if int(session.get("_gate5nc_rejections") or 0) >= 1:
        if _tail_is_other and not _says_the_name:
            # CA50437bb292 (14 Sep 2026, demo): "so that's Emma Ojma. I've got
            # you on 0750… is that the best number?" → "no wrong again wrong
            # again" — twice — and neither counted, because the tail asked
            # about the number. But Susie read a NEW name back in that same
            # breath, and "wrong" (not a bare "no") after a fresh read-back
            # is about the name. CAd63554bb's "uh no it's not" stays a number
            # answer: it has no "wrong"-family word.
            if _read_back and _REJECTION_WRONG_RE.search(_c):
                return True
            return False
...
_REJECTION_WRONG_RE = re.compile(
    r"\bwrong\b|\bnot right\b|\bincorrect\b|\bstill (?:wrong|not)\b|\bnot it\b",
    re.IGNORECASE,
)
...
def sanitise_response(text: str, session: Dict[str, Any]) -> str:
    ...
    # ── Gate 5n-f: one word given, asked for the other half twice → exit ────
    # CA1cdbe346b4 (14 Sep 2026, demo): "gardner" → "and your first name?" →
    # "gardner" → "could I take your first name as well?" → "gardner" → "is
    # Gardner your first name or your surname?" → "my full name gardner
    # gardner" → "so that's Gardner Gardner noted". Four asks for the other
    # half; 5n-d counted none (no "didn't catch / again" in them). The owner's
    # rule is one re-ask, then the best effort goes on the booking and the
    # spelling is chased by text. So: while the name on record (or the
    # caller's best effort) is ONE word, every model turn that asks for a name
    # part counts; the second one is replaced by the exit, with that word.
    # "Thanks Gardner — and your surname?" is ask #1 and is always spoken.
    # Subsumes the surname-is-the-first-name-again case (CA29c06f3309): a
    # same-word reply is one more turn with one word on record.
    if (
        not _nc_exit_spoken
        and not session.get("_gate5n_exited")
        and not session.get("booking_confirmed")
        and _NAME_PART_ASK_RE.search(result)
    ):
        _nf_word = _stored_name_token(session)
        _nf_known_full = " " in (
            (session.get("patient_name") or "")
        ...
        if _nf_word and not _nf_known_full:
            ...
            if session.get("_gate5nf_seen") != _nf_key:
                session["_gate5nf_seen"] = _nf_key
                session["_gate5nf_asks"] = int(session.get("_gate5nf_asks") or 0) + 1
                logger.info(
                    "[ms_gate5nf] asked for the other half of a one-word name "
                    "(#%d, have %r): %r",
                    session["_gate5nf_asks"], _nf_word, result[:60],
                )
                if session["_gate5nf_asks"] >= 2:
                    result = _gate5n_exit(session, _nf_word.capitalize())
                    _nc_exit_spoken = True
                    logger.info(
                        "[ms_gate5nf] second ask — exit spoken; %r on the booking, "
                        "SMS confirmation flagged, asked instead: %r",
                        _nf_word, result[-60:],
                    )
