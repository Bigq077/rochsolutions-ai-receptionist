...
#
# ⚑ theorem-onboarding: ON. THIS IS THE BRANCH IT HAPPENED TO.
#
# This branch descends from latency-eval, which defaults OFF because it is an
# isolated timing-eval service that must never text a real caller. That default
# came across with the lineage — straight past a comment in this very spot that
# said, in as many words, "DO NOT port this default flip to live branches".
# It was ported anyway (3b2f195, 2026-08-04). Mark's line then sent nothing: no
# booking confirmation, no staff transfer notice, no reminder. Every acceptance
# call logged "[sms] SMS_ENABLED is off" and read as correct, because that is
# exactly what a healthy eval branch prints.
#
# Worse than silence, it made Susie a liar. The theorem_v3 prompt promises "I've
# just sent you a confirmation text" UNCONDITIONALLY — it has no SMS gate of its
# own at all — so every caller was told about a text that was never coming.
# That unconditional promise is still there; it is only safe while this default
# is ON. Treat switching SMS off on this branch as a caller-facing change.
#
# ON means a forgotten Render env var fails in the safe direction for a LIVE
# clinic, which is sending. Set SMS_ENABLED=false to silence deliberately.
# Do NOT copy latency-eval's "false" here.
