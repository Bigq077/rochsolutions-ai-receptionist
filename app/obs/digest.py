...
def _needed_callback(call: dict) -> bool:
    """True for calls the judge said a human should ring back.

    These were texted to the operator the moment the call ended, so they are NOT
    "review" calls — judge.needs_review() deliberately excludes them. The daily
    email must still list them: they are the most serious calls of the day, and a
    report that silently omits them would claim a bad day was clean.
    """
    from app.obs import judge
    return judge.needs_callback(call)
...
def build_email(calls: List[dict], hours: int) -> tuple:
    ...
    """
    callbacks = [c for c in calls if _needed_callback(c)]
    ...
    subject = (f"Susie daily — {len(calls)} call(s), {s['booked']} booked, "
               f"{len(callbacks)} callback(s), {len(review)} to review ({hours}h)")
    ...
    def _detail(c: dict) -> List[str]:
        out = []
        tags = ", ".join(c.get("failure_tags") or []) or "—"
        out.append(f"  [{c.get('quality_score')}/5] {c.get('call_sid')}  ({tags})")
        if c.get("evidence"):
            out.append(f"      {c['evidence']}")
        if config.OBS_DIGEST_INCLUDE_TRANSCRIPTS:
            out.extend(_transcript_lines(c))
            out.append("")
        return out

    rows: List[str] = []

    # Callbacks first — the day's most serious calls. Already texted when they
    # happened; repeated here so the daily record is complete.
    if callbacks:
        rows += ["", f"NEEDED A CALLBACK — {len(callbacks)} (texted at the time):"]
        for c in _worst_first(callbacks):
            rows += _detail(c)
    ...
    if review:
        ...
        rows += ["", "Nothing needed a human this window."]
    else:
        rows += ["", "Calls to review: none beyond the callback(s) above."]

    rows += ["", "All calls (worst first):"]
    if calls:
        for c in _worst_first(calls):
            score = c.get("quality_score")
            if _needed_callback(c):
                flag = "  <-- CALLBACK"
            elif _needs_attention(c):
                flag = "  <-- review"
            else:
                flag = ""
