...
def _spine(clinic: Dict[str, Any], tk: Dict[str, str], dc: Dict[str, str]) -> Dict[str, str]:
    ...
    reschedule_cancel = (
        ...
        "- If the caller has ALREADY clearly asked to RESCHEDULE (they said "
        "'reschedule', 'move it', 'change the time', or similar), do NOT ask "
        "which they want — go STRAIGHT to the RESCHEDULE branch below.\n"
        "- Only if the caller asked to CANCEL, or their intent is genuinely "
        "unclear, ask exactly: 'Would you like to cancel this appointment "
        "altogether, or would you like to reschedule it to a different time?' "
        "Wait for their answer, then follow the matching branch below.\n\n"
