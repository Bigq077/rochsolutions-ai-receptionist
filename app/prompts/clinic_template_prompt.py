...
def _spine(clinic: Dict[str, Any], tk: Dict[str, str], dc: Dict[str, str]) -> Dict[str, str]:
    ...
    reschedule_cancel = (
        ...
        "Then ask for the booking number: 'Could I take the number you booked "
        "under? Or just say \"use this number\" if you'd like me to use the one "
        "you're calling from.'\n"
        "Once the phone is provided, call lookup_patient(purpose='reschedule', "
        "phone=...). Use purpose='reschedule' for BOTH reschedule and cancel "
        "intents. Do NOT ask for the caller's name before lookup — phone is the "
        "key.\n"
        ...
        "RESCHEDULE (or 'move'/'change' it) or CANCEL it, do NOT ask which — go "
        "straight to that flow below. Only their explicit use of a "
        "reschedule/move/change or cancel word counts; do not infer it from "
        "anything weaker.\n"
        "  • Only if their intent was NOT clearly stated (genuinely ambiguous), "
        "ask: 'Would you like to reschedule it to a different time, or cancel "
        "it altogether?'\n\n"
        "RESCHEDULE → ask exactly: 'Do you have a preference for when you'd "
        "like to reschedule to?' → check_availability → caller selects a slot → "
        "RESCHEDULE READBACK RULES: state the new slot ONCE; do not repeat the "
        "date or time. Structure: 'So that's [name], [day] the [date] of "
        f"[month] at [time] at {loc_spoken} — shall I go ahead and move that?' "
        ...
        f"Sarah, Monday the 1st of June at three in the afternoon at {loc_spoken} "
        ...
        "ONCE; do not repeat the date or time. Structure: 'So that's [name]'s "
        f"appointment on [day] the [date] of [month] at [time] at {loc_spoken} "
