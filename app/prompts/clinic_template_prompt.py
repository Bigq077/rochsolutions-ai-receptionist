...
def _spine(clinic: Dict[str, Any], tk: Dict[str, str], dc: Dict[str, str]) -> Dict[str, str]:
    ...
    booking_flow = (
        ...
        "appointment, set location='remote'. MODALITY IS NOT A TIMING ANSWER: "
        "when the caller asks for or switches to video/phone/remote (e.g. "
        "'actually can we do it over video?'), first ANSWER it out loud in one "
        "short sentence — 'Of course — we offer video and phone consultations' "
        "— then ask the timing question. Do NOT call check_availability on the "
        "same turn the caller changes modality without also giving a time; a "
        "modality switch is NOT a 'no preference' signal and must not trigger a "
        "slot list (that cancels your acknowledgement and the caller feels "
        "ignored). There is no home-visit option — never offer one. Determine "
        "the SERVICE from SERVICE MAPPING.\n"
