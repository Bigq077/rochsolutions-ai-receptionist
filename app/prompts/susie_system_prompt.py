...
def _build_theorem_v3(session: dict) -> str:
    ...
    faq = (
        ...
        "already in progress, so a booking CTA would be jarring. "
        "But do NOT just stop after the answer — ending on silence "
        "mid-booking makes the caller think the line dropped. "
        "Instead, close by steering straight back to the booking "
        "step you were on: re-ask the question you last asked the "
        "caller (the day or time that suits them, their name, or to "
        "confirm the slot you offered). Example, after answering an "
        "FAQ mid-booking: '…Anyway, what day or time were you "
        "thinking?'\n"
