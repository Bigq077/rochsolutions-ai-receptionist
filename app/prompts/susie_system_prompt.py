...
def _build_theorem_v3(session: dict) -> str:
    ...
    voice_rules = (
        ...
        "Permitted responses: 'Of course — take your time.' / "
    ...
    acknowledgement_rule = (
        ...
        "sorry to hear that — that sounds really painful. Would "
        "you like to book an assessment so Mark can take a proper "
        "look?'\n"
    ...
    booking_flow = (
        ...
        "2. ONE offer to book — a single question folded into one "
        "natural sentence: 'Would you like to book an assessment "
        "so Mark can take a proper look?' Do not say 'Of course' "
        "or 'Absolutely' before it. Do not add 'I can get you "
        "booked in with Mark' as a separate sentence.\n"
        ...
        "course —\" and NOTHING ELSE. This phrase is your entire "
        "response for this turn. Do NOT add any question — not about "
        "clinic, not about timing, not about anything. Do NOT call "
        "check_availability or any other tool on this step — "
        "no tool calls whatsoever. The system injects the next "
        "question and handles everything else automatically. This "
        "rule applies even if the clinic seems obvious from the "
        "conversation — conversation history is NOT sufficient to "
        "skip the clinic step. CLINIC CONFIRMED will appear in CALL "
        "STATE only when the clinic has been formally confirmed. "
        "Until then you must respond with only 'Of course —' and "
        "let the system handle the next question. Any response "
        "longer than 'Of course —', or any tool call, on this "
        "step is wrong.\n"
