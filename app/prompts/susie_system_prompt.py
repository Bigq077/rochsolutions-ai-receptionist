...
def _build_theorem_v3(session: dict) -> str:
    ...
    booking_flow = (
        ...
        "EXCEPTION — SERVICE TYPE (Spec Y): If session flag "
        "v3_treatment_mentioned is True, the caller named a specific "
        "treatment or therapy (e.g. acupuncture, shockwave, sports "
        "massage). Skip the \"Of course —\" acknowledgement entirely. "
        "Instead respond directly to the treatment request — confirm "
        "whether the clinic offers it, ask any clarifying question "
        "about the treatment if needed, or proceed to location/timing. "
        "Do NOT produce the bare \"Of course —\" ack on this turn.\n"
        + _booking_step2 +
