...
def _build_theorem_v3(session: dict) -> str:
    ...
    booking_flow = (
        ...
        "clinic, not about timing, not about anything. The system "
        "injects the next question automatically. This rule applies "
        "even if the clinic seems obvious from the conversation — "
        "conversation history is NOT sufficient to skip the clinic "
        "step. CLINIC CONFIRMED will appear in CALL STATE only when "
        "the clinic has been formally confirmed. Until then you must "
        "respond with only 'Of course —' and let the system handle "
        "the next question. Any response longer than 'Of course —' "
        "on this step is wrong.\n"
