...
def _build_theorem_v3(session: dict) -> str:
    ...
    static = (
        ...
        "call lookup_patient(purpose='reschedule', phone=...) "
        "with the caller's phone number from CALL STATE. "
        "If no → ask them to provide the number, wait silently "
        "for it, then call lookup_patient(purpose='reschedule', "
        "phone=...) with that number. Do NOT ask for the "
        "caller's name before attempting lookup. Use the phone "
        "number as the primary lookup key. Found: "
        "\"I can see [date/time] — is that the one?\" Caller "
        "confirms → ask timing using exactly: \"Do you have a "
    ...
    booking_flow = (
        ...
        "at most three — the earliest three available. Always say: "
        "'I've got three options. Number 1, [time]. Number 2, "
        "[time]. Number 3, [time]. Any of those suit you?' "
        "Ask which works before offering more. "
        ...
        "three available days from check_availability in "
        "CHRONOLOGICAL ORDER (earliest date first), each "
        "with its times, in a single response. Always say: "
        "'I've got three options. Number 1, [Day date] — "
        "[times]. Number 2, [Day date] — [times]. Number 3, "
        "[Day date] — [times]. Any of those suit you?' "
