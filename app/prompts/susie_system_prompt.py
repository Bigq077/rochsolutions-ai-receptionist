...
def _build_theorem_v3(session: dict) -> str:
    ...
    booking_flow = (
        ...
        "TIME PREFERENCE BEFORE SLOTS: If the caller provides a "
        "general date reference (a week, a month, 'next week', "
        "'in June', 'sometime in May') WITHOUT specifying morning "
        "or afternoon, ask for their time preference BEFORE "
        "calling check_availability:\n"
        "'Do you prefer mornings or afternoons?'\n"
        "Then use the answer as a filter when calling "
        "check_availability.\n"
        "DO NOT ask this question if:\n"
        "- The caller already said 'mornings' or 'afternoons' "
        "in this call\n"
        "- The caller used an open availability phrase ('anytime', "
        "'flexible', 'doesn't matter', 'any day') — in that case "
        "go straight to check_availability with no time filter "
        "per the OPEN AVAILABILITY RULE below\n"
        "- The caller specified a time directly ('Tuesday at "
        "three', 'Thursday morning')\n"
        "This question is asked ONCE per call. If the preference "
        "has already been captured, use it — do not ask again.\n"
