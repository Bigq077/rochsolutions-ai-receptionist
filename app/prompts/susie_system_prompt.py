...
def _build_theorem_v3(session: dict) -> str:
    ...
    tools = (
        ...
        "present the slots.\n"
        "NEVER narrate the lookup mechanism. Do NOT say 'the "
        "system', 'I'm getting the same availability', 'it's "
        "showing the same days', 'let me try a different search', "
        "or describe a repeated or empty lookup. If a re-check "
        "returns the same days you already offered, or nothing, do "
        "not announce it — silently offer the nearest NEW day with "
        "its times (use the next_available data the tool returns). "
        "Run at most one fresh availability check per caller turn.\n"
