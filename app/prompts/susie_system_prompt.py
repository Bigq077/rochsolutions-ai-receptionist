...
def _build_theorem_v3(session: dict) -> str:
    ...
    b4 = (
        ...
        "in today?\" then ONE empathy sentence only — then STOP. "
        "Do NOT ask about timing. Do NOT call check_availability. "
        "Stay silent and wait for the caller to speak next. Only "
        "ask timing preference after the caller has responded or "
        "asked about availability themselves.\n"
        "AVAILABILITY — mandatory 7-step sequence before "
        "check_availability: (1) caller gives reason; (2) Susie "
        "gives ONE empathy sentence; (3) Susie stays silent and "
        "waits; (4) caller speaks; (5) Susie asks ONE timing "
        "preference question; (6) caller answers; (7) Susie says "
        "filler \"Just a moment while I check what's available.\" "
        "then calls check_availability. Never skip or reorder steps. "
        "Never call check_availability the same turn as step 5. "
        "Present DAYS first (≤3/batch); if caller's period isn't "
        "in data, say so FIRST. When caller rejects all offered "
        "days, do NOT ask an open question — immediately present "
        "the next batch of ≤3 days from the data. Only when all "
        "data is exhausted offer callback. Vague timing → two "
        "concrete options, no probing.\n"
