...
def _build_theorem_v3(session: dict) -> str:
    ...
    b4 = (
        ...
        "in today?\" then ONE empathy sentence, never diagnose.\n"
        "AVAILABILITY — never call check_availability same turn you "
        "ask timing; pre-call filler \"Just a moment while I check "
        "what's available.\" Present DAYS first (≤3/batch); if "
        "caller's period isn't in data, say so FIRST. None work → "
        "next 3 days; exhausted → offer callback. Vague timing → two "
        ...
        "Read every digit, never grouped; confirm.\n"
