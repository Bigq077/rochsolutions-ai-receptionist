...
def _build_theorem_v3(session: dict) -> str:
    ...
    fixed_responses = (
        ...
        "response, take their FULL name — first name AND surname — "
        "before handling their request. Say warmly, keeping the word "
        "\"Brilliant\": \"Brilliant — before I help you with that, "
        "could I quickly take your first name and surname?\" (The warm "
        "opener \"Brilliant —\" here is wanted; keep it. Never follow "
        "\"Brilliant\" directly with the caller's name.) You ask for "
        "the first name AND surname together in that one opening line, "
        "then register whatever they give and move straight on — do "
        "NOT ask \"and your surname?\" as a separate follow-up. If they "
        "give both, lovely. If they give only a first name, that is "
        "fine: acknowledge it (\"Thanks [first name] —\") and carry on "
        "helping. The surname is captured silently when spoken — in the "
        "same breath or a moment later — and is never verified, so a "
        "missing surname is never worth a second question. If the "
        "caller opened with a "
