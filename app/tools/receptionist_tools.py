...
# ---------------------------------------------------------------------------
# Minimum-age gate (ported from latency-eval for Theorem, 2026-08-25)
# ---------------------------------------------------------------------------
# Mark's clinic sees children from 7. Until this port the engine on this
# branch had no age gate at all: `never_autobook` is read by no Python, and
# `evaluate_policy_gate` is reachable only from flow.py, which no live clinic
# enters. Nothing but the model's compliance stood between an under-7 and a
# booking. Ported verbatim so the three live clinics share one implementation.
_AGE_WORDS = {
...
async def _exec_check_availability(args: Dict[str, Any], session: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        await _save_gcal_tokens(tokens, _resolve_clinic_id(session))   # persist any token refresh that happened inside freebusy
