...
def _all_day_busy_blocks(events, w_start, w_end) -> list:
    ...
    Ported from the Vital Edge diary reader (`ddd5318`), where it was wired only
    into `_check_availability_diary`. The hazard is not VE-specific: every
    clinic on the ordinary Google Calendar path has the same hole, and the
...
async def _exec_check_availability(args: Dict[str, Any], session: Dict[str, Any]) -> Dict[str, Any]:
    ...
    _out = _cap_presented_slots(_filter_same_day_slots(_payload, session), session)
