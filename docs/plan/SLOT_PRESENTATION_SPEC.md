...
| D-t (engine) | 13 Sep 12:31 (CAdd1bdd17, `bef29667`) — on the reworded prompt the model said the echo aloud (*"Tuesdays and Thursdays after four — just a moment while I check"*) and did NOT wait; same lookup, same retraction. The prompt does not hold the rule. | **enforced in the engine, unproved on a call**: `_timing_unearned_this_turn` (llm_stream) refuses `check_availability` with `ask_timing_first` unless this turn's words carry a timing signal, the hint is urgency the caller said, or the previous turn was Susie's own timing/echo question; on a yes to the echo, the echo AND the hint are folded into the guard's asked-set (D). `test_dt_a_lookup_needs_a_timing_the_caller_gave_this_turn.py`. Also from that call: L2 (history match by clause; `v3_confirmed_slot_phrase` / `last_spoken_slot_phrase` gated by `was_retracted`) and E2 (the name question inside the replaced chunk is kept) — `e540d1bc`. |
| D (same-turn bound) | 13 Sep 12:58 (CAdb28a1a6, `5d28332e`) — the chain now works end-to-end through the echo path (D-t BLOCKED a lookup on *"yeah that'd be quite a walk"*, echo asked, clean one-slot offer, `caller ACCEPTED` on the yes, correct read-back), but the FIRST lookup on *"um yes after 4 on a tuesday"* was still retracted: `requested_clock_times` reads a bound as no time by design, the model's hint `Tuesday after 4pm` reads as 16:00, the builder said "nearest to four", the guard had no 16:00. | **fix on `latency-eval`, unproved**: on the gate's same-turn allow path, when the caller's own words carry a bound WITH an hour (`_BOUND_WITH_HOUR_RE`), the hint is folded into the guard's asked-set; a day alone or a bare yes folds nothing (pinned). Also: the fold on a yes narrowed to the ECHO question only. Parser and producers untouched. The one-slot sentence for a bound (*"nearest to four"* rather than a readout of after-four times) is C proper — an owner presentation decision, left open. |
| DT-18 (one slot) | 13 Sep 13:16 (CAafb7f031, `86b77625`, an ORDINARY ASAP booking) — *"yeah the wednesday at 8 works"* accepted; follow-up *"eight in the morning on Wednesday is free. Shall I book that in for you?"*; *"uh yeah go for it"* → resolver None (two Wednesday times heard, no label in the words) → the re-query guard stood down → the model's spurious `check_availability` fell to the dedup branch → N4 read Wednesday's OTHER times with eight left out → *"why'd you take it away"*. NOT a regression from the 12–13 Sep chain (path unchanged; the 10 Sep call with the same words was spared only because the model did not re-query). | **fix on `latency-eval`, unproved**: resolver step 1b — ONE slot on the table AND it is what Susie said last (`_offer_is_what_susie_just_said`, from `conversation_history`) AND the words accept (step 0) and carry no negator, other weekday or other time → that slot. Decided on STATE, not a vocabulary of yeses (owner: *"if somebody confirms in a non-traditional way he is stuck"*). A yes to a later question (parking) pins nothing. Belt-and-braces: `utterance_accepts_offered_slot` strips leading fillers and knows "go for it". |

| DT-7/8 (accept) | 13 Sep 21:53 (CA4b4afa80, northgate, `103d0ae3`) — *"yeah 20 to 10 on a wednesday works"*: `caller ACCEPTED 09:40 — pinned (P6b)`, then in the SAME turn the DT-7/8 route answered the same words as a question: *"Yes — twenty to ten on Wednesday 16th is free. Shall I book that in for you?"* An accept became an offer; the caller accepted twice; the second yes earned a `slot_picked` head. Introduced by `d585450a` (11 Sep), widened by D-r `e161c435`; on every clinic line since. Owner, 14 Sep: a clear pick goes straight to *"So that's … — could I take your first name and surname?"*, no head | **closed** (14 Sep): `accepted_slot_readback_speech` — a pinned accept is read back from the engine's record (D-s) ABOVE every producer, ~150 ms, no model, no head; a QUESTION shape falls through to DT-7/8 as before (the resolver pins *"is twenty to ten still free?"* — found writing the test); a MIXED utterance goes to the model. Test `test_an_accepted_slot_is_read_back_not_offered_again.py`. Proof call owed |
