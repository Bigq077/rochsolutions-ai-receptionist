...
# ── Hold speech (added 2026-08-29, mined from the 733-call corpus) ──────────
# The engine's own deterministic hold phrases, lower-cased. A transcript line
# matching one EXACTLY was spoken by the engine rather than generated by the
# model, which is what makes these four detectors decidable from obs alone.
_HOLD_PHRASES = {p.lower() for p in [
...
# A welded sentence boundary: the model runs its own opener straight into the
# payload and ElevenLabs reads it without a breath.
_RUN_ON = re.compile(r"[a-z][.!?][A-Z]")

# A subordinate clause left standing alone after its head was stripped. The
# terminator must be a FULL STOP -- an earlier version of this allowed "?" and
# matched 103 calls, every one of them an ordinary question ("What's the
# appointment for?"). Requiring the full stop takes it to 5, which is the real
# figure and matches the six such fragments recorded live on 21-22 Aug.
...
def d_hold_dead_end(call):
    """A hold phrase was the last thing said before the caller spoke again.

    "Just getting that for you…" and then nothing. The phrase promised the
    caller something was coming and it never came -- 39 calls in the corpus.
    """
    turns = _turns(call)
    for i, turn in enumerate(turns):
        ...
        if text.lower() not in _HOLD_PHRASES:
            continue
        for j in range(i + 1, len(turns)):
            if (turns[j].get("role") or "") == "user":
                break
            nxt = (turns[j].get("text") or "").strip()
            if nxt and nxt.lower() not in _HOLD_PHRASES:
                return None
        return f"{text!r} was the last thing said"
    return None
...
def d_run_on_sentence(call):
    """The model's opener welded to the payload, spoken without a breath.

    "…what's available.The available slots for Tuesday". join_after_head is
    meant to repair this; until 2026-08-29 its replacement was a bare space, so
    it deleted the full stop AND the capital instead of inserting a space.
    """
    for text in _bot(call):
        m = _RUN_ON.search(text or "")
        if m:
            start = max(0, m.start() - 30)
            return f"…{text[start:m.start() + 30]}…"
    return None


def d_dangling_clause(call):
    """A subordinate clause spoken as if it were a sentence.

    "While I look that up." -- what is left when the opener strip removes
    "Bear with me" and nothing checks the remainder. `_ORPHAN_LEAD` guards this
    and listed only the adverbial words, so "Let me check what's available for
    Saturday" -> "What's available for Saturday." went straight through it.
    """
    for text in _bot(call):
        m = _DANGLING.search(text or "")
        if m:
            return m.group(0)[:60]
    return None


def d_provisional_claimed_a_write(call):
    """A pending-confirmation clinic said the booking was done.

    "Just locking that in now…" and then "sent it to Jonathan… subject to his
    confirmation". Nothing was locked in. work_for_tool routes book_appointment
    to PENDING_REQUEST on a provisional clinic to make this unreachable; this
    detector is what proves it stays unreachable.
    """
    joined = " ".join(_bot(call)).lower()
    if "subject to" not in joined:
        return None
    for claim in ("locking that in", "booked in for you", "popping that in the diary"):
        if claim in joined:
            return f"said {claim!r} on a pending-confirmation booking"
    return None
...
DETECTORS = [
    ...
    # ── Hold speech. Added 2026-08-29 from a corpus mine; the file had carried
    # the same 7 detectors since 31 Jul while defects ran to B-119.
    Detector("B-120", "a hold phrase was the last thing said", "open",
             d_hold_dead_end,
             "CAc776e1218dcc2845ddff7f070f03b176"),
    Detector("B-121", "two hold phrases back to back", "open",
             d_hold_stacked,
             "CAbc6e6a96270e88d63feb69498d5a8b94"),
    Detector("B-122", "opener welded to the payload, spoken without a breath",
             "open", d_run_on_sentence,
             "CA7efc2c8c37f4cb8efb3a38d7b246f9bc"),
    Detector("B-123", "a subordinate clause spoken as a sentence", "open",
             d_dangling_clause,
             "CAac066043e6223f3165303c94c3246e7b"),
    Detector("B-124", "pending-confirmation clinic claimed the booking was done",
             "open", d_provisional_claimed_a_write,
             "CAbc6e6a96270e88d63feb69498d5a8b94"),
]
...
def main() -> int:
    ...
    print(f"{'id':7}{'defect':56}{'n':>4}  {'last seen':22}build")
    print("-" * 100)
    for det in DETECTORS:
        if not det.hits:
            print(f"{det.id:7}{det.what:56}{0:>4}  {'-- not observed --':22}")
            continue
        last = sorted(det.hits)[-1]
        print(f"{det.id:7}{det.what:56}{len(det.hits):>4}  "
